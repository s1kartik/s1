		...
		if( isset($_POST['no_options']) && "1" == $_POST['no_options'] ) {
			...
			foreach($elements as $el) {
				if( "procedure_duediligence" == $regno_section ) {
					echo '<option value="'.$el['st_regulation_number'].'">'.$el['st_regulation_number'].'</option>';
				}
				else {
...
/********end hrq marcio codes**********/
	...
	// Messages section with new designs received on 06Nov2014 //
		function message_metro()
		{
			...
			$total_inbox_messages 	= $this->users->getMyMessages( $this->sess_useremail, 0, '', array('assign','alert') );
			...
			// Inbox Messages //
				$data['rows_inbox_messages']= $this->users->getMyMessages($this->sess_useremail, ($pgno_inbox-1), $this->msg_rows_limit, array('assign','alert'));
				// common::pr($data['rows_inbox_messages']);
		...
		function courses()
		{
			...
			if( $union_id ) {
				...
				// Get AVERAGE GRADE with the average grade percentage of all users who attended Union Courses;
					$average_grade = ($tot_average_grade_ratio/$cnt_participants);
					(!$average_grade) ? $average_grade = '0': '';
