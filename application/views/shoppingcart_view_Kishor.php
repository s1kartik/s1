  ...
  <div class="container-fluid" style="padding-bottom:30px">
    ...
    ?>
        ...
        <form method="post" id="cartfrm" action="<?php echo $payment_url;?>">
          ...
          <!--<div class="clearfix overFlowSec">
               	<table width="100%" class="table bg-black fg-white" id="checkout-table" >-->
          <div class="clearfix">
            <table class="table table-striped table-bordered table-condensed table-hover" id="checkout-table" >
              <tbody>
                <?php $i=1;
              ...
              <thead>
                <tr>
                  <th width="80" class="span1">Remove</th>
                  <th width="50" class="span1">Item</th>
                  <th >&nbsp;</th>
                  <th width="50" >Credits</th>
                  <th width="100">Quantity</th>
                  <th width="100" ><div class="<?php echo $cls_total_credits;?> fg-yellow"><i class=""><img src="<?php echo $base;?>img/icons_s1credit.png" style="height:20px;" /></i>&nbsp;<?php echo $total_available_credits;?></div></th>
                  <th class="textright">Price</th>
                  <th width="100" class="textright">Subtotal</th>
                </tr>
              </thead>
              <tr id="row<?php echo $item['id'];?>" >
                <td data-title="Remove"><a class="remove-row martmi12" title="Remove from your cart" rel="<?php echo $item['rowid'];?>" href="#"><i class="icon-remove-sign"></i></a></td>
                
                <td data-title="Item"><!--?php echo $item['library_type'];?--><span class="icon" ><img src="<?php echo $base;?>img/<?php echo $icon;?>.png" width="30" height="30"></span></td>
                
                <td style="border-bottom:none !important;"><?php echo $item['name'].$item_label;?></td>
                ...
                <td data-title="Total Credits"><input type="radio" name="rdb_payment_option[<?php echo $rowid;?>]" value="moneris" <?php echo isset($item['payment_option'])&&$item['payment_option']=='moneris'?"checked":"checked";?>>
                  ...
                  <?php /*
								if( !isset($item['transaction_type']) ) {?>
									<input type="radio" <?php echo $disabled_credits_payment;?> name="rdb_payment_option[<?php echo $rowid;?>]" value="s1credits" <?php echo isset($item['payment_option'])&&$item['payment_option']=='s1credits'?"checked":"";?>>S1 Credits
								<?php 
								}*/?></td>
                <td data-title="Price" class="price"><?php echo number_format($item['price'], 2);?></td>
                <td data-title="Subtotal" class="subtotal"><?php echo number_format($item['subtotal'], 2);?></td>
          ...
          <div class="checkout-area">
            <button type="button" onclick="javascript:location.href='<?php echo $base;?>admin/profile';" class="keep link" style="float: left;">Keep Shopping</button>
            <button class="link update">Update Your Cart</button>
            <button class="danger " type="button" id="id_btn_checkout">Checkout</button>
          </div>
        </form>
        <?php 
	}?>
