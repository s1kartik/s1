createAccordions = function(){
	$("div.metro-accordion").children("h3").each(function(){
			...
			$(this).prepend("<img class='accordionArrow' src='"+PATH_FOLDER+"themes/"+theme+"/img/primary/arrowRight.png'/>");
					...
					$("div.metro-accordion").children("h3").each(function(){
						if($(this).text().toLowerCase() == t){
							...
							if($.browser.name=="msie" && $.browser.version<9){
								$(this).children("img").attr("src",""+PATH_FOLDER+"themes/"+theme+"/img/primary/arrowBottom.png")
		...
		$("#content, #panelContent").children("div.metro-accordion").on("click","h3",function(){
			...
			if($c.css("display") == "none"){
				if($d.hasClass("hide-others")){
					...
					$d.children("h3").children("img").attr("src",""+PATH_FOLDER+"/themes/"+theme+"/img/primary/arrowRight.png").removeClass('down').addClass("right");
				...
				if($.browser.name=="msie" && $.browser.version<9){
					$(this).children("img").attr("src",""+PATH_FOLDER+"presentation/themes/"+theme+"/img/primary/arrowBottom.png")
				}else if($.layout.name == "webkit"){	
					r=0;
					$accordion.turnImageDown($(this).children("img"));
				}else{
				...
				$c.stop().slideUp(500);
				if($.browser.name=="msie" && $.browser.version<9){
					$(this).children("img").attr("src",""+PATH_FOLDER+"themes/"+theme+"/img/primary/arrowRight.png")
				}else if($.layout.name == "webkit"){
					r=90;
